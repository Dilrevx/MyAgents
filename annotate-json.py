...
def do_with_text(input_text):
    # 运行链并获取结果
    result: AnnotateMeta = chain.invoke({"text": input_text})

    # --- 7. 打印结果 ---
    print("✅ 提取成功！")
    print(f"标题 (title): {result.title}")
    print(f"类型(type): {result.type}")
    print(f"应用场景 (scenario): {result.scenario}")
    print(f"分析方法 (method): {result.method}")
    print(f"简称 (short_name): {result.short_name}")

    # 你也可以直接打印 Pydantic 对象
